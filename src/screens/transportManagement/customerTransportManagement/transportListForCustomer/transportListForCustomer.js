...
function TransportListForCustomers() {
    ...
    const filteredTransport = transport && transport.filter((customerTransportList) => {
        return customerTransportList.startingStation.toLowerCase().startsWith(startingStation.toLowerCase())
    ...
    return(
        <div className="mainD">
            ...
            <div className="d-flex justify-content-between align-items-center">
                <div className="col-md-5" style={{ marginLeft: '200px' }}>
                    <div className="form-group">
                        <input type="text" className="form-control rounded-pill" id="startingStation" 
                        placeholder="Search by Starting Station"
                        value={startingStation} onChange={handleStartingStationChange} />
                    </div>
                </div>
            ...
            <div className="row">
                {filteredTransport && filteredTransport.map((customerTransportList)  => (
                <div key={customerTransportList._id} className="bus-detail-box">
                    ...
                    <div className="column-left">
                        <p><span style={{ fontWeight: 'bold' }}>Starting Station:</span> {customerTransportList.startingStation}</p>
                        <p><span style={{ fontWeight: 'bold' }}>Destination Station: </span>{customerTransportList.destinationStation}</p>
                        <p><span style={{ fontWeight: 'bold' }}>Mobile Number: </span>{customerTransportList.mobileNo}</p>
                        <p><span style={{ fontWeight: 'bold' }}>Leaving Time: </span>{customerTransportList.leavingTime}</p>
                        <p><span style={{ fontWeight: 'bold' }}>Total Travel Time: </span>{customerTransportList.totalTravelTime}</p>
                        
                    </div>
                    <div className="column-right">
                        <p><span style={{ fontWeight: 'bold' }}>Facilities : </span>
                            {customerTransportList.facilities.join(", ")}</p>
                        <p><span style={{ fontWeight: 'bold' }}>City Stops : </span>
                            {customerTransportList.cityStops.join(", ")}</p>
                            <p><span style={{ fontWeight: 'bold' }}>Total Number Of Seats: </span>{customerTransportList.totalNumberOfSeats}</p>
                        <p><span style={{ fontWeight: 'bold' }}>Ticket Price (LKR):</span>{customerTransportList.ticketPrice}</p>
                    </div>
                </div>
                ))}
               </div>     
